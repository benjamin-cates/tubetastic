...
const get_captions_for_video = async () => {
    const video_id = (document.getElementById("video-title-link") as HTMLAnchorElement).href.replace("https://www.youtube.com/watch?v=","");
    console.log("getting captions for",video_id);
    fetch("https://youtube.com/watch?v="+video_id+"#heyo").then(response=>response.text().then(console.log));
    // const content = await get_captions(video_id);
    // console.log(content);
